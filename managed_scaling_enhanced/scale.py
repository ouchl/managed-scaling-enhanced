...
def scale_in(cluster: Cluster, dry_run: bool = False) -> bool:
    ...
    if cluster.task_cpu_count == 0:
        ...
        if scale_in_step > 0:
            ...
            logger.info(f'Setting managed scaling policy max units to {new_max_units}.')
            cluster.modify_scaling_policy(max_units=new_max_units)
            logger.info(f'Setting task fleet target OD capacity to {new_od_capacity}.')
            logger.info(f'Setting task fleet target spot capacity to {new_spot_capacity}.')
            if not dry_run:
